//! llmtop — local LLM and agent monitor.
//!
...
pub fn run() -> io::Result<()> {
    ...
    if std::env::args().any(|a| a == "--version" || a == "-V") {
        println!("llmtop {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
...
fn run_update() -> io::Result<()> {
    ...
    println!("llmtop v{current} — checking for updates...\n");
    ...
    let dl_status = std::process::Command::new("curl")
        .args([
            ...
            "--tlsv1.2",
            "-LsSf",
            "https://github.com/weby-homelab/LLMtop/releases/latest/download/llmtop-installer.sh",
            "-o",
        ])
        .arg(&installer_path)
        .status()?;
    ...
    if !dl_status.success() {
        ...
        eprintln!("  cargo install llmtop --force");
        std::process::exit(1);
    }
